...
import { PortalSidebar, MobilePortalSidebar } from './portal-sidebar'
import { PortalHeader } from './portal-header'
import { cn } from '@/lib/utils'
import { usePortalAuthGuard } from '@/lib/portal-auth'
...
const pathToBreadcrumb: Record<string, string> = {
  ...
  '/portal/documents/upload': 'Upload Documents',
  '/portal/financials': 'Financial Overview',
  '/portal/messages': 'Messages',
  '/portal/invoices': 'Invoices & Payments',
  '/portal/invoices/pay': 'Make Payment',
...
function generateBreadcrumbs(pathname: string): BreadcrumbItem[] {
  ...
  const breadcrumbs: BreadcrumbItem[] = [{ name: 'Dashboard', href: '/portal' }]
...
export function PortalLayout({ children, className }: PortalLayoutProps) {
  ...
  const { session, isLoading } = usePortalAuthGuard()
    ...
    <div className={cn('min-h-screen bg-gray-50 dark:bg-gray-900', className)}>
      <div className="flex">
        ...
        <div className="flex-1 lg:pl-64 flex flex-col min-w-0">
          {/* Header */}
          <PortalHeader onMobileMenuToggle={handleMobileMenuToggle} />
          ...
          <footer className="px-6 py-4 border-t border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900">
            <div className="flex items-center justify-between text-sm text-gray-600 dark:text-gray-400">
              <div>
                © 2024 {session.organizationName}. Powered by CPA Platform.
              </div>
              <div className="flex items-center space-x-4">
                <a href="/portal/help" className="hover:text-gray-900 dark:hover:text-gray-200 transition-colors">
                  Help
                </a>
                <a href="/portal/help#privacy" className="hover:text-gray-900 dark:hover:text-gray-200 transition-colors">
                  Privacy
                </a>
                <a href="/portal/help#contact" className="hover:text-gray-900 dark:hover:text-gray-200 transition-colors">
                  Contact
                </a>
              </div>
            </div>
